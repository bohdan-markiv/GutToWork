...
// ----- Zod Schema for Poop Form -----
const formSchema = z.object({
    score: z.coerce.number().int("Score must be between 1 and 5"),
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-8" style={{ backgroundColor: 'white', color: 'var(--primary)' }}>
            ...
            <div className="w-full max-w-6xl overflow-auto border-2 border-[var(--background)] rounded-lg">
                <Table className="border-collapse">
                    ...
                    <TableBody>
                        {[...poop]
                            .sort((a, b) => new Date(b.poop_date).getTime() - new Date(a.poop_date).getTime())
                            .map((item) => (
                                <TableRow
                                    key={item["poop-id"]}
                                            ...
                                            <AlertDialog>
                                                ...
                                                <AlertDialogContent>
                                                    ...
                                                    <AlertDialogHeader>
                                                        ...
                                                        <AlertDialogDescription>
                                                            This action cannot be undone. It will permanently delete this poop record.
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
                                                    <AlertDialogFooter>
                                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                        <AlertDialogAction onClick={() => handleDelete(item["poop-id"])}>Delete</AlertDialogAction>
                                                    </AlertDialogFooter>
                                                </AlertDialogContent>
                                            </AlertDialog>
                                        </div>
            ...
            {selectedPoop && (
                <AlertDialog open={!!selectedPoop} onOpenChange={() => setSelectedPoop(null)}>
...
function EditForm({ poop, onSubmit, onCancel, successMessage, errorMessage }: EditFormProps) {
            ...
            <div>
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                        <DropdownMenuSeparator />
                        {["morning", "afternoon", "evening", "night"].map((type) => (
                            <DropdownMenuItem key={type} onSelect={() => setTimeOfDay(type)}>
                                {type}
                            </DropdownMenuItem>
                        ))}
                    </DropdownMenuContent>
                ...
                <div className="flex space-x-1 mt-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                        <button
                            ...
                            className={`text-2xl ${star <= score ? 'text-yellow-400' : 'text-gray-300'}`}
                        >
                            ★
                        </button>
                    ))}
